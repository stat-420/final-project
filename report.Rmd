...
model_additive = cached_lm(log(SalePrice) ~ 
...
model_additive_influential<-cached_lm(formula = log(SalePrice) ~  LotArea + X1stFlrSF + X2ndFlrSF + MasVnrArea + BedroomAbvGr + OverallQual + OverallCond  + ExterQual + KitchenQual + BsmtQual + BsmtExposure + ScreenPorch + Fireplaces + MSZoning + YearBuilt, data=train_data[-influential_indexes,])
...
dfs<-lapply(names(model_config),function(model_name){
  ...
  preds<-exp(predict(m,train_data))
  my_rsme<-ModelMetrics::rmse(train_data$SalePrice,preds)
